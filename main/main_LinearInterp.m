...
show_ECG_min_left = 800; % this the beginning of ECG, so keep some data if possible. I changed to 1 min 10/09/2020.
show_ECG_min_right = 800; % end of ECG, so keep more data if possible. I changed to 1 min 10/09/2020.
% >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
if HPC_flag
    addpath([filesep,'home',filesep,'doh16101',filesep,'MATLAB',filesep,'Pulsewatch_alignment',filesep,'path']);
else
    addpath(['..',filesep,'path']);
end
...
% >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
manual_add_sec = 0;% for 002B: -624+28+32 for seg, 3500
...
% manual_add_sec = -seconds(ECG_init_datetime + seconds(ECG_time_5min(1)) - PPG_time_5min(1)) - 110182/temp_intp_num;
...
['Suggested move sec: ', num2str(my_suggest_time_shift,'%2.4f'),' sec'],...
['You move sec: ', num2str(manual_add_sec,'%2.4f'),' sec']}; % title for the figure.

fprintf('Suggested move second: %f\n',my_suggest_time_shift);
